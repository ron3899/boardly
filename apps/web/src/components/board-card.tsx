...
export function BoardCard({ board, onDelete }: BoardCardProps) {
  ...
  return (
    <div className="group relative rounded-lg bg-card text-card-foreground border border-border shadow-sm hover:shadow-lg dark:hover:shadow-black/30 transition-all hover:-translate-y-1 duration-200">
      {/* Top colored strip */}
      <div
      ...
      <Link href={`/app/boards/${board.id}`} className="block p-5">
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-[15px] text-foreground mb-1 truncate">
              {board.name}
            </h3>
      ...
      <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
        ...
        >
          <Star
            className={`h-4 w-4 ${isFavorite ? 'fill-monday-warning text-monday-warning' : 'text-muted-foreground'}`}
          />
        </button>

        {/* Three-dot menu */}
        <DropdownMenu>
          <DropdownMenuTrigger className="p-1.5 rounded-lg hover:bg-accent transition-colors">
            <MoreHorizontal className="h-4 w-4 text-muted-foreground" />
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-40 bg-popover border border-border shadow-lg rounded-md">
            <DropdownMenuItem
